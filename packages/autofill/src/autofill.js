...
export function parseBeanText(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (out[field] && field !== "REGION" && field !== "MEMO") {
      ...
      if (
        SHORT_FIELDS.includes(field) &&
        next &&
        !findFieldForLabelExact(next) &&
        isLongText(next) &&
        !out.MEMO
      ) {
        out.MEMO = next;
  ...
  if (out.NET_WEIGHT) {
    ...
    if (m) {
      const grams =
        (m[2] || "").toLowerCase() === "kg" ? Math.round(parseFloat(m[1]) * 1000) : parseInt(m[1], 10);
  ...
  if (!out.TASTING_NOTE) {
    const m =
      /(?:tasting notes?|flavou?r(?:s)?(?:\s*notes?)?|cup(?:ping)? notes?)\s*[:\-–]?\s*(?:of\s+)?([A-Za-z][A-Za-z ,&'’]{6,80})/i.exec(
        text,
      );
