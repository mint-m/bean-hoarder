...
export const ROAST_LEVELS: readonly RoastLevel[] = [
  { agtron: 120, ko: "울트라라이트", swatch: "oklch(0.74 0.055 68)" },
  { agtron: 95, ko: "라이트", swatch: "oklch(0.63 0.072 58)" },
  { agtron: 75, ko: "미디움라이트", swatch: "oklch(0.53 0.078 48)" },
  { agtron: 65, ko: "미디움", swatch: "oklch(0.44 0.070 42)" },
  { agtron: 55, ko: "미디움다크", swatch: "oklch(0.35 0.052 38)" },
  { agtron: 45, ko: "다크", swatch: "oklch(0.26 0.033 35)" },
];

/** 저장 형식 — 폼·AI·라벨이 모두 이 형태를 만든다. 바꾸면 기존 행과 어긋난다. */
export function roastLevelValue(l: RoastLevel): string {
  return `#${l.agtron} (${l.ko})`;
}
...
/** AI 프롬프트가 쓰는 6단계 나열 — "#120(울트라라이트), #95(라이트), …" */
export function roastLevelsForPrompt(): string {
  return ROAST_LEVELS.map((l) => `#${l.agtron}(${l.ko})`).join(", ");
}
