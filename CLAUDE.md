...
시드 계정은 `TEST`·`DEMO`(둘 다 암호 `0000`)이고 **로컬 전용**이다. `TEST`는 e2e가 쓰고, `DEMO`는
데모에 실을 원두를 랩에서 꾸며 보는 저작용이다 — 구경거리 자체는 계정이 아니라 정적 페이지(`/demo`)라
...
- **`setState(updater)` 안에서 바깥 변수를 채우고 곧바로 읽지 말 것.** React는 대기 중인 업데이트가
  ...
  **updater 바깥에서 순수하게 계산**한 뒤 넘긴다 — `Workspace.fillParsed` 참고. 최신 폼은 렌더마다
  갱신하는 `formRef`로 읽는다.
- **`db/seed.sql`은 로컬 픽스처다 — 원격 D1에 실행하지 말 것.** `TEST`·`DEMO`의 암호가 저장소에
  적혀 있어, 원격에 넣으면 누구나 로그인하는 계정이 라이브에 생긴다. **서비스에 특별 취급되는
  계정은 없다** — 예전엔 공개 데모 계정을 두고 서버가 쓰기를 막았지만, 그 구조는 계정을 관리할
  방법까지 함께 없앴다. 지금 시드의 `DEMO`는 구경거리의 출처가 아니라 그 콘텐츠를 앱에서 꾸미기
  위한 **저작용 계정**이다 — 데모 페이지는 D1을 읽지 않으므로 라이브에 이 계정이 없어도 된다.
- **데모는 DB가 아니라 콘텐츠다 — D1과 이어 붙이지 말 것.** 덱(`/demo`)도 카드(`/DEMO…`)도
  ...
  내용을 바꾸려면 로컬 랩에 시드의 `DEMO` 계정으로 로그인해 꾸민 뒤 `npm run gen:demo-beans`로
  떠 온다(로컬 D1 → JSON, 단방향). KEY가 유저코드에서 나오므로 계정 이름이 `DEMO`여야 한다.
