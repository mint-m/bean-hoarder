...
- **데모 데이터는 한 방향으로만 흐른다 — 되짚어 올라가지 말 것.**
  `원격 D1 → apps/web/src/demo-beans.json → db/seed.sql → 로컬·e2e D1`.
  덱(`/demo`)은 정적이지만 카드를 누르면 열리는 상세는 **실제 공개 조회(`/{KEY}`)**라 원격 D1이
  답한다 — 그래서 권위가 라이브에 있고, JSON은 정적 덱을 빌드하기 위한 사본이다.
  중간의 두 파일은 **생성물이라 손으로 고치지 않는다**(`npm run gen:demo-beans`가 둘 다 갱신하고
  `npm run check`가 어긋남을 잡는다). 데모 내용을 바꾸라는 요청은 **랩에서 DEMO 계정으로 라이브
  원두를 고친 뒤 떠 오는** 일이다.
  로컬 D1에서 떠 오지 않는 이유: 로컬은 이 사슬의 끝이라 거기서 되떠 오면 고리가 닫혀 JSON이
  원본도 사본도 아니게 되고, 로컬에서 아무리 꾸며도 라이브 카드는 그대로라 데모가 거짓이 된다.
  KEY를 바꾸면 데모 링크가 죽으므로 KEY는 함부로 갈지 않는다.
