...
시드 계정은 `TEST` / `0000` 하나뿐이고 **로컬·e2e 전용**이다. 데모는 계정이 아니라 정적 페이지
(`/demo`)라 로그인이 필요 없다 — 아래 "데모는 DB가 아니라 콘텐츠다" 항목 참고.
...
- **데모는 DB가 아니라 콘텐츠다 — D1과 이어 붙이지 말 것.** 덱(`/demo`)도 카드(`/DEMO…`)도
  ...
  **그 JSON이 데모 콘텐츠가 사는 유일한 곳이다** — 저작용 D1 계정도 내보내기 스크립트도 두지
  않는다. 한때 "랩에서 꾸미고 떠 오기"를 뒀다가 걷어냈는데, 같은 내용이 로컬 D1과 JSON 두 곳에
  살면서 JSON이 어떤 때는 원본이고 어떤 때는 사본이 됐기 때문이다(게다가 CI는 남의 로컬 D1을
  볼 수 없어 어긋남을 검사할 수도 없다). 손으로 쓰는 대신 폼이 봐 주던 검증은
  `apps/web/src/demo-beans.test.ts`가 맡는다 — 필수 항목·KEY 형식·접두·중복·필드명 오타.
