...
2. **등록** — 원두 정보를 입력하면 라벨이 실시간 미리보기되고(QR은 렌더링 때마다 203dpi 실디코드 자동 검증), 등록 버튼을 누르면 서버가 KEY(`{유저코드}{연도}-{순번}`)를 채번. 로스터리·산지·로스팅일·패키징일·용량은 필수, 산지·가공방식은 옵션 제안(블렌드 포함), 고도·수확시기는 접어서 필요할 때만 입력.
